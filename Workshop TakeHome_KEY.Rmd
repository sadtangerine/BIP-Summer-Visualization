---
title: "R you Ready ? Visualizing in R Practice Answer Key"
...
1. What is the variable type for `Species`? **Factor!**

2. Base on the variable type, what aesthetic would be the most appropriate to use to visualize `Species`? **We can use color and shape as the factor only has 3 levels so we won't exhaust the color and shape options. You can think of factors as categorical variables (or similar to them) and the same way we would visualize a categorical variable is the same way we can visualize factors!** 
...
```{r}
## using color to visualize species
iris %>% 
  ggplot(aes(x = Sepal.Length, y = Sepal.Width)) +
...
```{r}
## using shape to visualize species
iris %>% 
  ggplot(aes(x = Sepal.Length, y = Sepal.Width)) +
...
#### We can use both color and shape to visualize species
...
iris %>% 
  ...
  geom_point(aes(color = Species,
                 shape = Species),
             size = 3.5 ) + ## making the points bigger
  theme_bw()
```


How would you describe the relationship between `Sepal.Length` and `Sepal.Width` now that `Species` is included?
We can see that the relationship between Sepal.Length and Sepal.Width is moderated by Species in that different species cluster together. 
